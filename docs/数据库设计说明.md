...
erDiagram
    ...
    tblUser ||--o{ tblBookWish : recommends
    tblUser ||--o{ tblReservation : requests
    tblUser ||--o| tblReservationPatron : reservation_stats
    tblBook ||--o{ tblReservation : reserved_as
    tblBookCopy |o--o{ tblReservation : targeted
...
好书推荐同样引用 `tblUser.userId`；批准引进后把 ISBN 写回推荐记录，不建书目外键，
以免写入书目失败时留下悬挂引用。
预约委托引用 `tblUser.userId`；批准后写入命中的 `copyId`，归还后该副本状态为 `HELD`。
违约次数和停权截止保存在 `tblReservationPatron`，不写进每条预约。
...
| firstAttemptCapacity | INTEGER | 非空 | 首修严格名额上限；与重修之和 ≤ 最大容量，未分配部分为管理员预留缓冲 |
| retakeCapacity | INTEGER | 非空 | 重修严格名额上限；与首修之和 ≤ 最大容量，未分配部分为管理员预留缓冲 |
...
| attemptType | TEXT(8) | 非空 | `FIRST` 首修或 `RETAKE` 重修 |
...
修读类型由服务端按该生是否存在该课程的历史成绩记录（`tblCourseRecord`）判定；
选课时只检查本人所属名额池，池满即拒绝。首修与重修名额上限之和不要求等于最大容量，
差额作为管理员预留缓冲，不开放学生自助选课。

`tblCourseSection` 的 `classTime` / `location` 保留为兼容显示摘要；实际排课以
`tblSectionSchedule` 为准，每个教学班可有多条“周范围 + 星期 + 节次 + 地点”。

### 3.14 tblSectionSchedule（教学班时段）

| 字段 | Access 类型 | 约束 | 说明 |
| --- | --- | --- | --- |
| scheduleId | TEXT(32) | 主键 | 时段编号 |
| sectionId | TEXT(24) | 外键、非空 | 所属教学班 |
| weekday | INTEGER | 非空 | 星期（1=周一 … 7=周日） |
| periodStart / periodEnd | INTEGER | 非空 | 起止节次（闭区间） |
| weekStart / weekEnd | INTEGER | 非空 | 起止周（闭区间） |
| location | TEXT(64) | 可空 | 上课地点 |

冲突判定规则：两条时段在“同星期 + 节次区间重叠 + 周范围有交集”时视为冲突；选课、
管理员调课保护与教师时段占用检查统一使用该规则。

### 3.13 tblCourseRecord（修读 / 成绩记录）

| 字段 | Access 类型 | 约束 | 说明 |
| --- | --- | --- | --- |
| recordId | TEXT(32) | 主键 | 记录编号 |
| studentId | TEXT(20) | 外键、非空 | 学生学号 |
| courseId | TEXT(20) | 外键、非空 | 课程（目录）编号 |
| semesterName | TEXT(32) | 非空 | 修读学期 |
| gradeStatus | TEXT(16) | 非空 | `通过` / `未通过` 等 |

`(studentId, courseId, semesterName)` 唯一。修读记录当前由选课模块维护演示数据，
待成绩模块接入后转交；存在任意该课成绩记录的学生再次选课即按“重修”分流。
...
| copyStatus | TEXT(16) | 非空 | `AVAILABLE`、`BORROWED` 或 `HELD`（锁定保管） |
...
| renewCount | INTEGER | 非空 | 已续借次数，0 到 2 |

### 3.12.1 tblBookWish

| 字段 | Access 类型 | 约束 | 说明 |
| --- | --- | --- | --- |
| wishId | COUNTER | 主键 | 推荐编号 |
| userId | TEXT(32) | 外键、非空 | 提交人登录账号，引用 `tblUser.userId` |
| title | TEXT(128) | 非空 | 许愿书名 |
| author | TEXT(64) | 非空 | 许愿作者 |
| status | TEXT(16) | 非空 | `PENDING`、`APPROVED` 或 `REJECTED` |
| submitTime | TEXT(19) | 非空 | 提交时间，固定 `yyyy-MM-dd HH:mm:ss` |
| reviewTime | TEXT(19) | 可空 | 审核时间 |
| reviewerUserId | TEXT(32) | 可空 | 审核人登录账号 |
| isbn | TEXT(32) | 可空 | 批准引进后写入的书目 ISBN |

同一用户对相同书名和作者只能有一条待审推荐。演示库预置教师待审《三体》和学生已拒绝《百年孤独》。

### 3.12.2 tblReservation

| 字段 | Access 类型 | 约束 | 说明 |
| --- | --- | --- | --- |
| reservationId | COUNTER | 主键 | 预约编号 |
| userId | TEXT(32) | 外键、非空 | 申请人登录账号，引用 `tblUser.userId` |
| isbn | TEXT(32) | 非空 | 目标书目 ISBN |
| copyId | LONG | 可空 | 批准后命中的副本；待审时为空 |
| status | TEXT(16) | 非空 | `PENDING`、`APPROVED`、`HELD`、`PICKED_UP`、`REJECTED` 或 `EXPIRED` |
| applyTime | TEXT(19) | 非空 | 申请时间，固定 `yyyy-MM-dd HH:mm:ss` |
| reviewTime | TEXT(19) | 可空 | 审核时间 |
| reviewerUserId | TEXT(32) | 可空 | 审核人登录账号 |
| holdUntilTime | TEXT(19) | 可空 | 锁定保管截止时间 |
| pickupTime | TEXT(19) | 可空 | 取书时间 |

进行中的预约为 `PENDING`、`APPROVED`、`HELD`。同一用户对同一 ISBN 只能有一条进行中的申请。不预置演示预约。

### 3.12.3 tblReservationPatron

| 字段 | Access 类型 | 约束 | 说明 |
| --- | --- | --- | --- |
| userId | TEXT(32) | 主键、外键 | 读者登录账号，引用 `tblUser.userId` |
| defaultCount | INTEGER | 非空 | 预约违约次数，0 到 3 |
| suspendUntil | TEXT(19) | 可空 | 停权截止时间；空值表示未停权 |

第三次违约时写入 `suspendUntil`（当时起 30 天）。停权期满后申请新预约时清零违约次数。
...
- 已有学生的班级不得删除，应先停用。
- 有选课、借阅、预约、推荐、购物车或订单记录的学生不得删除；有授课、借阅、预约、推荐、购物车或订单记录的教师不得删除。
- 班级所属院系必须与学生所属院系一致。
