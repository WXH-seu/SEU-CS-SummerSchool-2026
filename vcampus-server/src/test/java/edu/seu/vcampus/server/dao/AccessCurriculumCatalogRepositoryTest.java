...
public class AccessCurriculumCatalogRepositoryTest {
    ...
    public void createsCatalogAndImportsReviewedSnapshotIdempotently() throws Exception {
        ...
        assertTrue(computerCourses.get(0).getRecommendedYear() > 0);
        assertTrue(computerCourses.get(0).getRecommendedSemester() > 0);
        ...
        AcademicService academicService = new AcademicService(academics, users);
        try {
            academicService.deleteDepartment("admin", SubSystemRole.ADMIN, "AI");
            fail("Department with a catalog major should be protected");
