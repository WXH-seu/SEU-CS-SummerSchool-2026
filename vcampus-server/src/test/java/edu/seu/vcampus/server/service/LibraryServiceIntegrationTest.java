...
public class LibraryServiceIntegrationTest {
    ...
    public void adminCanCreateUpdateDeactivateAndDeleteBooks() throws Exception {
        BookDto created = service.saveBook("admin", SubSystemRole.ADMIN, new BookDto(
                "9787300000001", "测试图书", "测试作者", "测试出版社", "计算机", 2, true));
        assertEquals(2, created.getTotalCopies());
        assertEquals(11, service.queryBooks("student", SubSystemRole.STUDENT, null).size());

        BookDto updated = service.saveBook("admin", SubSystemRole.ADMIN, new BookDto(
                "9787300000001", "测试图书（修订）", "测试作者", "测试出版社", "计算机", 3, true));
        assertEquals("测试图书（修订）", updated.getTitle());
        assertEquals(3, updated.getTotalCopies());

        service.saveBook("admin", SubSystemRole.ADMIN, new BookDto(
                "9787300000001", "测试图书（修订）", "测试作者", "测试出版社", "计算机", 3, false));
        assertEquals(10, service.queryBooks("student", SubSystemRole.STUDENT, null).size());
        assertEquals(11, service.queryBooks("admin", SubSystemRole.ADMIN,
                new BookQueryRequest("", true)).size());
        assertEquals(10, service.queryBooks("student", SubSystemRole.STUDENT,
                new BookQueryRequest("", true)).size());

        service.deleteBook("admin", SubSystemRole.ADMIN, "9787300000001");
        assertEquals(10, service.queryBooks("admin", SubSystemRole.ADMIN,
                new BookQueryRequest("", true)).size());
    }
    ...
    public void studentAndTeacherCannotMaintainCatalog() throws Exception {
        ...
        try {
            service.saveBook("student", SubSystemRole.STUDENT, book);
            fail("Student should not maintain books");
        } catch (BusinessException expected) {
            assertEquals(ResponseCode.FORBIDDEN, expected.getResponseCode());
        }
        try {
            service.saveBook("teacher", SubSystemRole.TEACHER, book);
            fail("Teacher should not maintain books");
        } catch (BusinessException expected) {
            assertEquals(ResponseCode.FORBIDDEN, expected.getResponseCode());
        }
        try {
            service.deleteBook("student", SubSystemRole.STUDENT, "9787020024759");
    ...
    public void cannotDeleteOrShrinkBooksWithBorrowHistory() throws Exception {
        try {
            service.deleteBook("admin", SubSystemRole.ADMIN, "9787020008735");
        ...
        BookDto reduced = service.saveBook("admin", SubSystemRole.ADMIN, new BookDto(
                "9787111544937", "计算机网络（第7版）", "谢希仁", "电子工业出版社", "计算机", 1, true));
        assertEquals(1, reduced.getTotalCopies());
        service.deleteBook("admin", SubSystemRole.ADMIN, "9787020024759");
        assertEquals(9, service.queryBooks("student", SubSystemRole.STUDENT, null).size());
    }
