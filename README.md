...
- 自动导入经审核的东南大学公开院系、专业和课程数据快照。
- 登录、会话创建和登出。
- 学生、教师、院系和班级查询。
- 管理员维护学生、教师、院系和班级。
- 学生仅查看本人学籍并维护性别、出生日期、电话和邮箱；教师只读，管理员可维护。
- 学院和班级使用级联下拉选择；管理员可设置班级容量并批量导入新生学籍，系统自动分班。
- 课程查询、选课、退课和个人课表查询。
- 课程目录 / 教学班分层，受众按“全校 / 院系 × 入学年份区间”控制可见与可选。
- 选课时间窗口、课程性质（必修 / 限选 / 任选 / 通选）与教师选课名单。
- 未到 / 已过选课窗口、容量满、已选、时间冲突时保持可见但给出原因。
- 服务端按课程粒度加锁，避免并发抢名额造成超卖。
- 教师仅查看本人教学班，管理员维护教学班的服务端权限控制。
- 五个必做模块的客户端导航和协议操作入口。
- 图书检索、详情查看、借阅、归还、续借、逾期限制、个人借阅记录，管理员对书目、馆藏和全部在借记录的维护与查看，以及预约委托和好书推荐。
- 校园商店商品浏览、购物车、下单扣库存、订单查询与管理员维护。
...
- [图书馆模块测试记录](docs/图书馆模块测试记录.md)
- [系统使用说明](docs/系统使用说明.md)
- [东南大学公开数据快照](docs/东南大学公开数据快照.md)
